...
const instagram = {
  ...
  login: async (username, password) => {
    ...
    await instagram.page.waitForXPath(
      '//*[@id="react-root"]/section/main/article/div/div/div/form/div[4]/div/div[1]/input'
    );
    ...
    await instagram.page.waitForXPath(
      '//*[@id="react-root"]/section/main/article/div/div/div/form/div[6]'
    );
    saveinfo = await instagram.page.waitForXPath(
      '//*[@id="optIntoOneTap"]/label/div[2]'
    );
    if (saveinfo[0]) {
      loginButton_new = await instagram.page.$x(
        '//*[@id="react-root"]/section/main/article/div/div/div/form/div[7]/button/div'
      );
      loginButton_new[0].click();
    } else {
      loginButton = await instagram.page.waitForXPath(
        '//*[@id="react-root"]/section/main/article/div/div/div/form/div[6]/button'
      );
    ...
    try {
      ...
      Ads_cancel_button = await instagram.page.$x(
        "/html/body/div[3]/div/div/div[3]/button[2]"
      );
      if (Ads_cancel_button[0]) await Ads_cancel_button[0].click();
    } catch (error) {
      console.log(error);
    }
  },
  extract_users: async (message, followingURL) => {
    ...
    Appbutton = await instagram.page.$x(
    ...
    const newPagePromise = new Promise((x) =>
      instagram.browser.once("targetcreated", (target) => x(target.page()))
    ); // declare promise
  ...
  sendMessages: async (
    ...
    try {
      ...
      MyProfilButton = await instagram.page.$x(
        '//*[@id="react-root"]/section/nav[2]/div/div/div[2]/div/div/div[5]/a/span'
      );
  ...
  followFollwersOfAUser: async (numberToFollow, delay, user) => {
    ...
    try {
      ...
      followersButton = await instagram.page.$x(
        '//*[@id="react-root"]/section/main/div/ul/li[2]/a/span'
      );
    ...
    for (i = 1; i < numberToFollow; i = i + Math.floor(Math.random() * 5)) {
      followButton = await instagram.page.$x(
